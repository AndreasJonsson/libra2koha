bib_tables="$(mktemp)"
create_tables.pl --verbose --format="$SOURCE_FORMAT" --quote='"' --headerrows=$HEADER_ROWS --encoding=utf8 --specencoding=utf16 --ext=$TABLEEXT --spec="$SPECDIR" --columndelimiter="$COLUMN_DELIMITER" --escape="$ESCAPE_CHAR" --rowdelimiter="$ROW_DELIMITER" --dir "$tabledir" --table 'Items' --table 'BarCodes' --table 'StatusCodes' --table 'CA_CATALOG' --table 'LoanPeriods' > "$bib_tables"
