bib_tables="$(mktemp)"
create_tables.pl "${TABLE_PARAMS[@]}" --table 'CA_COPY' --table 'CA_COPY_LABEL' --table 'CA_COPY_TYPE' --table 'CA_NOT_AVAILABLE_CAUSE' --table 'CA_MEDIA_TYPE' --table 'CI_UNIT' --table 'CA_CATALOG' --table 'GE_LA_KEY' --table 'GE_LA_TXT' --table CI_CAT --table IL_LOAN --table IL_STATUS --table IL_LIBRARY --table CA_LOC --table CA_SUPPLIER --table LA_TXT > "$bib_tables"
